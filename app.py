...
df.columns = [
    'age', 'bmi', 'sun_hours_per_day', 'screen_time_hours',
...
# ── Save vitamin_d_ng_ml BEFORE any drops so visualizations can use it ──
vitamin_d_ng_ml_backup = df['vitamin_d_ng_ml'].copy()

# Binning / grouping columns kept on the raw df for visualisations
df['supplement_tier'] = pd.cut(
    df['vitamin_d_supplement_iu'],
    bins=[-1, 0, 400, 800, 1500, 2001],
    labels=['None (0 IU)', 'Low (1-400 IU)', 'Medium (401-800 IU)',
            'High (801-1500 IU)', 'Very High (>1500 IU)'],
    right=False
)
df['sun_hours_bins'] = pd.cut(df['sun_hours_per_day'],
...
df['sun_hours_quartile'] = pd.qcut(
    df['sun_hours_per_day'], q=4,
...
df['VitaminD_Supplement_Group'] = pd.cut(
    df['vitamin_d_supplement_iu'], bins=delay_bins, labels=delay_labels, right=False
)

# ---------------------------------------------------------------------------
# ENCODED DATAFRAME FOR MODELLING
# ---------------------------------------------------------------------------
df_encoded = df.copy()

# Drop the raw vitamin_d_ng_ml from the modelling frame
df_encoded = df_encoded.drop('vitamin_d_ng_ml', axis=1)
...
df_age_vitd_encoded = pd.get_dummies(
    df_encoded[['Age_Group', 'VitaminD_Supplement_Group']], drop_first=True
)
...
x = df_encoded.drop(columns=[
    'deficient', 'supplement_tier', 'sun_hours_bins',
    'sun_exposure_group', 'sun_hours_quartile', 'Age_Group',
...
logreg_model_for_predict = Pipeline([
    ('scaler', StandardScaler()),
    ('model', LogisticRegression(max_iter=1000, random_state=42))
])
...
for name, pipe in [
    ...
    if isinstance(pipe, CatBoostClassifier):
        cv_pipe = Pipeline([('model', CatBoostClassifier(
            iterations=300, learning_rate=0.1, depth=6,
    ...
    results[name] = {
        metric: f"{cv_res[f'test_{metric}'].mean():.3f} \u00b1 {cv_res[f'test_{metric}'].std():.3f}"
...
# ---------------------------------------------------------------------------
# GLOBAL REFERENCES USED INSIDE THE APP
# ---------------------------------------------------------------------------
x_full                    = x.copy()
...
def decision_curve(y_true, y_prob, thresholds):
    ...
    for pt in thresholds:
        ...
        nb = (TP / N) - (FP / N) * (pt / (1 - pt)) if (1 - pt) != 0 else (TP / N)
...
def run_streamlit_app():
    ...
    with tab1:
        ...
        # df_plot keeps vitamin_d_ng_ml for the plots that need it
        df_plot = df.copy()
        if 'vitamin_d_ng_ml' not in df_plot.columns:
            df_plot['vitamin_d_ng_ml'] = vitamin_d_ng_ml_backup.values
        df_plot['deficient'] = df_plot['deficient'].astype(str)

        # ── Viz 1 : Risk Surface Heatmap ─────────────────────────────────────
        st.subheader('Vitamin D Deficiency Risk Surface: Sun Hours vs. Supplementation')
        df_viz1 = df.copy()
        df_viz1['supplement_tier'] = pd.cut(
            df_viz1['vitamin_d_supplement_iu'],
            bins=[-1, 0, 400, 800, 1500, 2001],
            labels=['None (0 IU)', 'Low (1-400 IU)', 'Medium (401-800 IU)',
                    'High (801-1500 IU)', 'Very High (>1500 IU)'],
            right=False
        )
        df_viz1['sun_hours_bins'] = pd.cut(
            df_viz1['sun_hours_per_day'],
            bins=np.arange(0, 8.5, 0.5), right=False
        )
        risk_surface = (
            df_viz1.groupby(['sun_hours_bins', 'supplement_tier'])['deficient']
            .mean()
            .unstack()
        )
        # Convert Interval index to strings so matplotlib can handle them
        risk_surface.index   = risk_surface.index.astype(str)
        risk_surface.columns = risk_surface.columns.astype(str)
        # Fill any NaNs with 0 so the heatmap gets pure floats
        risk_surface = risk_surface.fillna(0).astype(float)

        overall_deficiency = df_viz1['deficient'].mean()
        fig1, ax1 = plt.subplots(figsize=(14, 10))
        sns.heatmap(
            risk_surface * 100, annot=True, fmt='.1f',
            cmap='RdYlGn_r', center=overall_deficiency * 100,
            linewidths=.5, linecolor='lightgrey',
            cbar_kws={'label': 'Deficiency Rate (%)'}, ax=ax1
        )
        ...
        # ── Viz 2 : Violin – Vitamin D by Deficiency Status ──────────────────
        st.subheader('Vitamin D Distribution by Deficiency Status')
        fig2, ax2 = plt.subplots(figsize=(10, 6))
        sns.violinplot(
            x='deficient', y='vitamin_d_ng_ml',
            data=df_plot,
            palette={'0': 'teal', '1': 'coral'},
            hue='deficient', legend=False, ax=ax2
        )
        ...
        ax2.set_xticks([0, 1])
        ax2.set_xticklabels(['Non-Deficient', 'Deficient'])
        ax2.legend()
        ...
        sns.histplot(
            data=df_plot, x='age', hue='deficient', kde=True,
            palette={'0': 'teal', '1': 'coral'},
            stat='density', common_norm=False, ax=ax3
        )
        ...
        # ── Viz 4 : Body Fat vs Vitamin D ────────────────────────────────────
        st.subheader('Body Fat Percentage vs. Vitamin D Levels')
        fig4, ax4 = plt.subplots(figsize=(10, 6))
        sns.scatterplot(
            x='body_fat_percentage', y='vitamin_d_ng_ml', hue='deficient',
            data=df_plot, palette={'0': 'teal', '1': 'coral'}, alpha=0.6, ax=ax4
        )
        ...
        # ── Viz 5 : Sun Exposure × Skin Tone violin ───────────────────────────
        st.subheader('Vitamin D Distribution Across Sun Exposure Levels and Skin Tone')
        df_viz5 = df.copy()
        if 'vitamin_d_ng_ml' not in df_viz5.columns:
            df_viz5['vitamin_d_ng_ml'] = vitamin_d_ng_ml_backup.values
        ...
        sns.violinplot(
            data=df_viz5, x='sun_exposure_group', y='vitamin_d_ng_ml',
            hue='skin_tone', palette='magma', split=False,
            inner='quartile', linewidth=1.5, ax=ax5
        )
        ax5.axhline(20, color='red', linestyle='--', linewidth=2, label='Deficiency Threshold')
        ax5.set_title('Vitamin D Distribution Across Sun Exposure Levels and Skin Tone',
        ...
        # ── Viz 6 : Stacked Bar – Skin Tone × Season ─────────────────────────
        ...
        # ── Viz 7 : Heatmap – Supplement Tier × Sun Quartile ─────────────────
        st.subheader('Heatmap of Median Vitamin D by Supplementation Tier and Sun Hours Quartile')
        df_viz7 = df.copy()
        if 'vitamin_d_ng_ml' not in df_viz7.columns:
            df_viz7['vitamin_d_ng_ml'] = vitamin_d_ng_ml_backup.values
        df_viz7['supplement_tier'] = pd.cut(
            df_viz7['vitamin_d_supplement_iu'],
            bins=[-1, 0, 400, 800, 1500, 2001],
            labels=['None', 'Low (<=400)', 'Medium (401-800)',
                    'High (801-1500)', 'Very High (>1500)'],
            right=False
        )
        df_viz7['sun_hours_quartile'] = pd.qcut(
            df_viz7['sun_hours_per_day'], q=4,
            labels=['Q1 (Low)', 'Q2', 'Q3', 'Q4 (High)']
        )
        median_vd_heatmap = (
            df_viz7.groupby(['supplement_tier', 'sun_hours_quartile'])['vitamin_d_ng_ml']
            .median()
            .unstack()
        )
        # Convert Categorical index/columns to strings
        median_vd_heatmap.index   = median_vd_heatmap.index.astype(str)
        median_vd_heatmap.columns = median_vd_heatmap.columns.astype(str)
        median_vd_heatmap = median_vd_heatmap.fillna(0).astype(float)

        fig7, ax7 = plt.subplots(figsize=(10, 8))
        sns.heatmap(
            median_vd_heatmap, annot=True, cmap='YlGnBu',
            fmt='.1f', linewidths=.5, linecolor='lightgrey', ax=ax7
        )
        ax7.set_title('Median Vitamin D (ng/mL) by Supplementation Tier and Sun Hours Quartile',
                      fontsize=16)
    ...
    with tab2:
        ...
        for y_prob, label in [
            ...
            (y_prob_xgb, f'XGBoost (AUC={roc_auc_score(y_test, y_prob_xgb):.3f})'),
            (y_prob_cat, f'CatBoost (AUC={roc_auc_score(y_test, y_prob_cat):.3f})'),
            (y_prob_tab, f'TabNet (AUC={roc_auc_score(y_test, y_prob_tab):.3f})'),
        ]:
        ...
        treat_all   = [prevalence - (1 - prevalence) * (pt / (1 - pt))
                       if (1 - pt) != 0 else prevalence for pt in thresholds]
        ...
        for y_prob, label in [
            ...
            ax_dca.plot(thresholds, decision_curve(y_test, y_prob, thresholds), label=label)
    ...
    with tab3:
        ...
        for col_name in columns_to_scale:
            ...
            input_data[col_name] = st.sidebar.number_input(
                f"Enter {col_name.replace('_', ' ').title()}",
        ...
        raw_vitamin_d_supplement_iu = st.sidebar.slider(
            "Vitamin D Supplement (IU)", min_value=0, max_value=2000,
            value=400, step=100, key="vitd_supp_slider"
        )
        ...
        age_group_raw      = pd.cut([raw_age], bins=age_bins_raw,
                                    labels=age_labels_raw, right=False)[0]
        vitamin_d_group_raw = pd.cut([raw_vitamin_d_supplement_iu], bins=delay_bins_raw,
                                     labels=delay_labels_raw, right=False)[0]
        ...
        # Build a zeroed input frame aligned to x_full columns
        ...
        for col, options in categorical_cols_original.items():
            selected_option = st.sidebar.selectbox(
                f"Select {col.replace('_', ' ').title()}", options, key=f"cat_{col}"
            )
            for option in options:
                ohe_col = f"{col}_{option}"
                if ohe_col in input_df.columns:
                    input_df.loc[0, ohe_col] = 1 if selected_option == option else 0

        # Fill Age_Group OHE columns
        for label in age_labels_raw:
            ohe_col = f"Age_Group_{label}"
            if ohe_col in input_df.columns:
                input_df.loc[0, ohe_col] = 1 if age_group_raw == label else 0

        # Fill VitaminD_Supplement_Group OHE columns
        for label in delay_labels_raw:
            ohe_col = f"VitaminD_Supplement_Group_{label}"
            if ohe_col in input_df.columns:
                input_df.loc[0, ohe_col] = 1 if vitamin_d_group_raw == label else 0
        ...
        if st.button("Predict Vitamin D Deficiency"):
            ...
            shap_values_individual = explainer_for_shap.shap_values(
                input_df_for_prediction.values
            )
            # shap_values may be 2-D (one row) – take [0] for the single sample
            if isinstance(shap_values_individual, list):
                sv_single = shap_values_individual[0][0]
                ev_single = explainer_for_shap.expected_value[0]
            else:
                sv_single = shap_values_individual[0]
                ev_single = explainer_for_shap.expected_value
            ...
            shap_vals_test = explainer_for_shap.shap_values(x_test_full.values)
            fig_bar, _ = plt.subplots(figsize=(10, 6))
            shap.summary_plot(shap_vals_test, x_test_full,
            ...
            fig_beeswarm, _ = plt.subplots(figsize=(10, 6))
            shap.summary_plot(shap_vals_test, x_test_full,
